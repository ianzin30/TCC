"""Evaluate ColPali v1.1 page retrieval against MMLongBench evidence pages.

Usage:
    uv run python evaluate_colpali_v1_1_pages.py

The evaluator renders each PDF page, embeds pages with ColPali v1.1, ranks
...
# The first ColPali download can stall on some macOS setups when the HF Xet
...
ARCH_NAME = "colpali_v1_1"
OUTPUT_STEM = "colpali_v1_1-retrieval-pages"
...
PDF_INDEX_RANGE_ENV = "COLPALI_PDF_INDEX_RANGE"
...
CACHE_DIR = SCRIPT_DIR / "cache" / "colpali_v1_1"
...
def _reload_colpali_adapter_weights(model: ColPali) -> dict[str, Any]:
    ...
    if missing_keys or shape_mismatches:
        raise RuntimeError(
            "Failed to remap all ColPali adapter weights: "
            f"missing={missing_keys[:5]}, shape_mismatches={shape_mismatches[:5]}"
        )
    if loaded_weights == 0:
        raise RuntimeError("No ColPali adapter weights were remapped.")
...
def _load_model() -> tuple[ColPali, ColPaliProcessor, str, torch.dtype]:
    ...
    model._adapter_reload_metadata = _reload_colpali_adapter_weights(model)
...
def _summarize(
    ...
    summary: dict[str, Any] = {
        ...
        "processing_duration_note": (
            "ColPali indexing duration includes PDF page rendering plus page "
