"""Evaluate ColQwen2.5 v0.2 page retrieval against MMLongBench evidence pages.

Usage:
    uv run python evaluate_colqwen2_5_v0_2_pages.py

The evaluator renders each PDF page, embeds pages with ColQwen2.5 v0.2, ranks
...
# Avoid stalled Hugging Face downloads on macOS setups where the Xet backend
# does not progress reliably for large model files.
...
ARCH_NAME = "colqwen2_5-v0.2"
OUTPUT_STEM = "colqwen2_5_v0_2-retrieval-pages"
...
PDF_INDEX_RANGE_ENV = "COLQWEN2_5_PDF_INDEX_RANGE"
...
CACHE_DIR = SCRIPT_DIR / "cache" / "colqwen2_5_v0_2"
...
def _target_adapter_key(source_key: str) -> str | None:
    ...
    if source_key.startswith(language_prefix):
        target_key = "language_model." + source_key.removeprefix(language_prefix)
...
def _reload_colqwen_adapter_weights(model: ColQwen2_5) -> dict[str, Any]:
    ...
    if missing_keys or shape_mismatches:
        raise RuntimeError(
            "Failed to remap all ColQwen2.5 adapter weights: "
            f"missing={missing_keys[:5]}, shape_mismatches={shape_mismatches[:5]}"
        )
    if loaded_weights == 0:
        raise RuntimeError("No ColQwen2.5 adapter weights were remapped.")
...
def _load_model() -> tuple[ColQwen2_5, ColQwen2_5_Processor, str, torch.dtype]:
    ...
    model = ColQwen2_5.from_pretrained(
        ...
        key_mapping={
            "^base_model\\.model\\.custom_text_proj": "custom_text_proj",
            "^model\\.embed_tokens": "language_model.embed_tokens",
            "^model\\.layers": "language_model.layers",
            "^model\\.norm": "language_model.norm",
        },
    ).eval()
    model._adapter_reload_metadata = _reload_colqwen_adapter_weights(model)
...
def _summarize(
    ...
    summary: dict[str, Any] = {
        ...
        "processing_duration_note": (
            "ColQwen2.5 indexing duration includes PDF page rendering plus page "
